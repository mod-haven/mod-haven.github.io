...
SetTimeout(1000, function()
    if table.type(Inv) == 'empty' then
        warn("No inventory resource was found, check mh_evidence/inventory.lua")
    end
end)

local ox = GetResourceState('ox_target'):find('start')
local qb = GetResourceState('qb-target'):find('start')

if ox then
    Target = { ox = true, exp = exports.ox_target }
elseif qb then
    Target = { qb = true, exp = exports['qb-target'] }
end
```
</details>
...
RegisterNetEvent('mh_evidence:Notify', Notify)

-- callback is in server/editable.lua
function HasItem(item)
    return lib.callback.await('mh_evidence:HasItem', 100, item)
end
...
function IsWearingGloves()
    ...
    if not Config.Gloves[model] then return false end

    if type(Config.Gloves[model]) == "table" then
        if lib.table.contains(Config.Gloves[model], armIndex) then
            return false
        end
    elseif type(Config.Gloves[model]) == "boolean" then
        return Config.Gloves[model]
    end
...
function CreateTarget(coords, options) end

if Target.ox then
    function CreateTarget(coords, options)
        return Target.exp:addSphereZone({
            coords = coords,
            radius = 0.5,
...
if Target.qb then
    function CreateTarget(coords, options)
        local name = ('Evidence_%s'):format(math.random(1, 999999))

        return Target.exp:AddCircleZone(name, coords, 0.5, {
            name = name,
            debugPoly = Config.DebugTarget,
        }, {
...
if Inv.core then
    ...
    lib.callback.register('mh_evidence:server:GetCurrentWeaponCore', function(source)
    ...
    if IsESX() then
        if Config.Features.FingerprintsVehicle then
            ESX.RegisterUsableItem('uv_flashlight', function(source)
                TriggerClientEvent('mh_evidence:CheckFingerprints', source)
            end)
            ESX.RegisterUsableItem('tweezers', function(source)
                TriggerClientEvent('mh_evidence:CheckForShellsInVeh', source)
            end)
            ESX.RegisterUsableItem('cleaning_kit', function(source)
                TriggerClientEvent('mh_evidence:DestroyFingerPrintsVeh', source)
            end)
        end

        if Config.Features.ShellsInVehicle then
            ESX.RegisterUsableItem('cleaning_shells', function(source)
                TriggerClientEvent('mh_evidence:CheckShellsVehicle', source)
            end)
        end
...
lib.callback.register('mh_evidence:HasItem', function(source, item)
    if Inv.ox then
...
lib.callback.register('mh_evidence:GetEvidenceByType', function(source, typee)
    local options = {}
...
lib.callback.register('mh_evidence:GetItemMetadata', function(source, type, caseId)
    if Inv.ox then
        local items = Inv.exp:GetInventoryItems(source)
...
RegisterNetEvent('mh_evidence:addBloodMetadata', function(weapon, target)
            ...
            if weapon and weapon.metadata then
                ...
                if not lib.table.contains(weapon.metadata.dna, name) then
                    ...
                    if Config.Notify.blood then
                        TriggerClientEvent('mh_evidence:Notify', src, L('Notify.have_blood'), 'warning')
                    end
                end
            end
        elseif Inv.qb then
...
RegisterNetEvent('mh_evidence:server:WeaponFingerprints', function(currentWeapon)
    local src = source
        ...
        if inventory then
            ...
            if item.category == 'weapons' then
                ...
                if Config.Notify.printsweapon then
                    TriggerClientEvent('mh_evidence:Notify', src, L('Notify.left_fp'), 'warning')
                end
            end
        end
    elseif Inv.codem then
...
Locales['en'] = {
    ...
    Target = {
        shell = 'Scann Shell Evidence',
        bullet = 'Scann Bullet Evidence',
        blood = 'Scann Blood Evidence',
        finger = 'Scann Fingerprint Evidence',
        wep = 'Scann Weapon',
        db = 'Open Database'
    }
}
