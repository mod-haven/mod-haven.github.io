...
### Add to server.cfg
Find your `resources` folder, inside create a new folder named `[modhaven]` if one doesn’t already exist, download asset from [Portal](https://portal.cfx.re/assets/granted-assets) and move into `[modhaven]`, and add `ensure [modhaven]` at the end of your `server.cfg` file. Make sure that all dependencies listed above are started before our asset.
</Steps>